package com.kirana.ui.shop.view

import android.os.Bundle
import android.support.v7.widget.DefaultItemAnimator
import android.support.v7.widget.LinearLayoutManager
import com.kirana.R
import com.kirana.data.network.Product
import com.kirana.ui.base.view.BaseActivity
import com.kirana.ui.shop.interactor.ShopMVPInteractor
import com.kirana.ui.shop.presenter.ShopMVPPresenter
import kotlinx.android.synthetic.main.activity_product_list.*
import javax.inject.Inject

class ShopActivity : BaseActivity(), ShopMVPView {

    @Inject
    internal lateinit var adapter: ProductAdapter

    @Inject
    internal lateinit var layoutManager: LinearLayoutManager

    @Inject
    internal lateinit var presenter: ShopMVPPresenter<ShopMVPView, ShopMVPInteractor>

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_shop)

        presenter.onAttach(this)

        var title = intent.getStringExtra("NAME")
        setToolbar(title)

        setUp()

    }

    private fun setUp() {
        layoutManager.orientation = LinearLayoutManager.VERTICAL
        recylerView.layoutManager = layoutManager
        recylerView.itemAnimator = DefaultItemAnimator()
        recylerView.adapter = adapter

        presenter.onViewPrepared("getProducts", "1")
    }

    override fun onFragmentAttached() {

    }

    override fun onFragmentDetached(tag: String) {

    }

    override fun displayProductList(list: List<Product>?) = list?.let {
        adapter.addProductToList(it)
    }

    override fun onDestroy() {
        presenter.onDetach()
        super.onDestroy()
    }
}
